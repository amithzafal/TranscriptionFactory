...
class cluster_full_genome():
	def __init__(self, outputDir, initFrame):
		...
		self.clusterFile = os.path.join(outputDir,str(r)+"_"+str(initFrame)+"_"+str(interval)+"total_cluster_full_genome.res")
	...
	def ProcessFrame(self, i):
		...
		self.cluster_number.extend([len(c) for c in sorted(nx.connected_components(G), key=len, reverse=True)])
		...
		for i in range(0,17):
			...
			self.cluster_number_cis.extend([len(c) for c in sorted(nx.connected_components(G_chrom), key=len, reverse=True)])
