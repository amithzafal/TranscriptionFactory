...
class PolyMSD():
	...
	def Compute(self):
			...
			for idxTad in range(self.Nchain):
				self.cumulDistHom += msdFFT(posHist[:, idxTad]-posHist[:, self.reader.SisterID[idxTad]])
					


	
	def ReadHist(self):
		posHist = np.zeros((1000-self.timepoint, self.reader.nTad, 3), dtype=np.float32)
		
		for i in range(1000-self.timepoint):
