...
class Forksnumber():
	...
	def __init__(self, outputDir,  initFrame, chrom,threshold=0.5, nMax=10, cutoff=1/2**0.5 + 1e-3):
		...
		self.TimingFile = os.path.join(self.reader.outputDir,"chrom_"+str(chrom)+ "_timing.res")
		self.percentageFile = os.path.join(self.reader.outputDir,"percentage.res")
			...
			for i in range(self.Nchain):
				if(self.Status[step][i]!=0):
					if(timing[i]<0):
						timing[i]=step
						step_perc+=1
			percentage.append(step_perc/self.Nchain)
		np.savetxt(self.TimingFile, timing)
		np.savetxt(self.percentageFile,percentage)
...
if __name__ == "__main__":
	...
	forksnumb = Forksnumber(outputDir,initFrame=initFrame,chrom=chrom)
