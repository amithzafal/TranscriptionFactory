...
for e in range(len(matric_names)):
	...
	mymatrix = rawdata
	if(matric_names[e]=="r_4.0_500_hic.cool"):
		print("g1")
		g1.append(avcopyweight)
		g1.append(mymatrix)
	if(matric_names[e]=="r_4.0_800_hic.cool"):
                print("s")
                s.append(avcopyweight)
                s.append(mymatrix)
...
bins = cooler.binnify(chromsizes, binsize)
bins["raw"]=1
avcopyweight=3*g1[0]+7*s[0]
bins["copyweight"]=1/(avcopyweight)
pixels = ArrayLoader(bins, 3*g1[1]+7*s[1], chunksize=10000000)
bins_dict["S_70"]=bins
pixels_dict["S_70"]=pixels


bins = cooler.binnify(chromsizes, binsize)
bins["raw"]=1
avcopyweight=4*g1[0]+6*s[0]
bins["copyweight"]=1/(avcopyweight)
pixels = ArrayLoader(bins, 4*g1[1]+6*s[1], chunksize=10000000)
bins_dict["S_60"]=bins
pixels_dict["S_60"]=pixels


bins = cooler.binnify(chromsizes, binsize)
bins["raw"]=1
avcopyweight=2*g1[0]+8*s[0]
bins["copyweight"]=1/(avcopyweight)
pixels = ArrayLoader(bins, 2*g1[1]+8*s[1], chunksize=10000000)
bins_dict["S_80"]=bins
pixels_dict["S_80"]=pixels


bins = cooler.binnify(chromsizes, binsize)
bins["raw"]=1
avcopyweight=1*g1[0]+9*s[0]
bins["copyweight"]=1/(avcopyweight)
pixels = ArrayLoader(bins, 1*g1[1]+9*s[1], chunksize=10000000)
bins_dict["S_90"]=bins
pixels_dict["S_90"]=pixels

bins = cooler.binnify(chromsizes, binsize)
bins["raw"]=1
avcopyweight=8*g1[0]+2*s[0]
bins["copyweight"]=1/(avcopyweight)
pixels = ArrayLoader(bins, 8*g1[1]+2*s[1], chunksize=10000000)
bins_dict["S_20"]=bins
pixels_dict["S_20"]=pixels
