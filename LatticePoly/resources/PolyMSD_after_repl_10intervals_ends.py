...
class PolyMSD():
	...
	def __init__(self, outputDir, initFrame):
		...
		self.msdHomFile = os.path.join(self.reader.outputDir, "MSD_after_repl.res")
	...
	def Compute(self):
			...
			for idxTad in range(self.reader.nTad):
				if self.reader.polyType[idxTad] == 1:
					...
					if(((idxTad<250 or idxTad>750) and idxTad<1000) or ((self.reader.SisterID[idxTad]<250 or self.reader.SisterID[idxTad]>750) and idxTad>=1000)):
	...
	def ReadHist(self):
		posHist = np.zeros((int((self.reader.N-self.timepoint)/10), self.reader.nTad, 3), dtype=np.float32)
				
		for i in range(self.reader.N-self.timepoint-10):
