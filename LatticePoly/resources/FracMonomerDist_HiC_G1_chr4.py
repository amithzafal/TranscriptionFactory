...
class MonomerDmap():
	def __init__(self, outputDir, initFrame):
		...
		self.contactFile = os.path.join(self.reader.outputDir,"r_"+str(r)+"_"+str(initFrame)+ "_G1_frac.cool")
...
#self.contactProb=np.rint(self.contactProb/finalFrame)
	...
	def ProcessFrame(self, i):
		...
		percent_func=lambda x: 1 / x
		frac = percent_func(r*0.71)
		inds = np.nonzero(np.random.random(len(data.polyPos[:])) < frac)[0]
		positions=data.polyPos
		for ind in inds:
			positions[ind][0]=np.nan
			positions[ind][1]=np.nan
			positions[ind][2]=np.nan
			
		tree1	= cKDTree(positions, boxsize = None)
