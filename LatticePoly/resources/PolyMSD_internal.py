...
class PolyMSD():
	...
	def __init__(self, outputDir, initFrame):
		self.reader = vtkReader(outputDir,1, initFrame, readLiq=False, readPoly=True)
		
		self.msdHetFile = os.path.join(self.reader.outputDir, "polyHetMSD.res")
		self.msdHomFile = os.path.join(self.reader.outputDir,"1polyMSD_internal.res")
		...
		if sizeTot < vMem.available:
			...
			for idxTad in range(1000):
				if self.reader.polyType[idxTad] == 1:
					...
					if((self.reader.SisterID[idxTad]>250 and self.reader.SisterID[idxTad]<750) or (idxTad>250 and idxTad<750)):
