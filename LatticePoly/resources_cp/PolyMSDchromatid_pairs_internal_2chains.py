...
class PolyMSD():
	...
	def __init__(self, outputDir, initFrame):
		self.Nchain=0
		self.reader = vtkReader(outputDir,0, initFrame, readLiq=False, readPoly=True)
		self.reader2=vtkReader(outputDir,1, initFrame, readLiq=False, readPoly=True)
		...
		self.msdHomFile = os.path.join(self.reader.outputDir, "MSD_pair_internal_2chain.res")
	...
	def Compute(self):
		...
		if sizeTot < vMem.available:
			...
			posHist2=self.ReadHist2()

			for idxTad in range(1000):
				if 1 == 1:
					if(idxTad>250 and idxTad<750):
						pos1=posHist[:, idxTad]
						pos2=posHist2[:, idxTad]
						diff=pos1-pos2
						self.cumulDistHom += msdFFT(diff)
	...
	def ReadHist(self):
		posHist = np.zeros((self.reader.N,1000, 3), dtype=np.float32)
