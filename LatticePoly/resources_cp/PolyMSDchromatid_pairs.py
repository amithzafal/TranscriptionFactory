...
class PolyMSD():
	...
	def __init__(self, outputDir, initFrame):
		...
		self.msdHomFile = os.path.join(self.reader.outputDir, "MSD_pair_internal.res")
	...
	def ReadHist(self):
		posHist = np.zeros((self.reader.N,2* self.Nchain, 3), dtype=np.float32)
		
		for i in range(self.reader.N):
			data = next(self.reader)
			posHist[i] = data.polyPos[:2*self.Nchain]
